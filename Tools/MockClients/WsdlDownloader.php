...
/*
 * Originally This file is part of the BeSimpleSoapClient.
 *
...
class WsdlDownloader
{
    ...
    public function download($wsdl)
    {
        ...
        if ($isRemoteFile || $this->resolveRemoteIncludes) {
            $cacheFilePath = $this->cacheFileFromWsdl($wsdl);
        ...
        if ($nodes->length > 0) {
            foreach ($nodes as $node) {
                ...
                if ($this->isRemoteFile($location)) {
                    $location = str_replace($this->cacheDir.DIRECTORY_SEPARATOR,'',$this->download($location));
            ...
            foreach ($nodes as $node) {
                if ($node->hasAttribute('schemaLocation')) {
                    ...
                    if ($this->isRemoteFile($schemaLocation)) {
                        $schemaLocation =  str_replace($this->cacheDir.DIRECTORY_SEPARATOR,'',$this->download($schemaLocation));
